...
public class JsonLevelDone
{
    ...
    public static void WriteIntToFile(string filePath, int data)
    {
        try
        {
            JsonLevelDone dataContainer = new JsonLevelDone();
            dataContainer.currentLevel = data;
    ...
    private static JsonLevelDone JsonToData(string jsonData)
    {
        JsonLevelDone dataContainer = JsonUtility.FromJson<JsonLevelDone>(jsonData);
